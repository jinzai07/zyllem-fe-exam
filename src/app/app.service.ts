...
export class ZyllemApiService {
  ...
  filterBy(): ArticleType {
    return this.filterBy$.getValue();
  }

  getAllArticlesByType(): Observable<FeaturedArticle[] | NormalArticle[]> {
    const articles: any[] = [
      {
        id: "1",
        ...
        featureImgUrl:
          "https://static.wixstatic.com/media/15e42d_62ed7c59561e452ead75691524ea4e1e~mv2.png/v1/fill/w_650,h_489,al_c,q_80,usm_0.66_1.00_0.01/15e42d_62ed7c59561e452ead75691524ea4e1e~mv2.webp",
      },
      {
        id: "2",
    ...
    return combineLatest(of(articles), this.filterBy$.asObservable()).pipe(
      map(([articles, filterBy]) => {
        if (filterBy === ArticleType.ALL) {
          return articles;
        }
        return articles.filter((article) => article.type === filterBy);
      })
    );
  }
}
